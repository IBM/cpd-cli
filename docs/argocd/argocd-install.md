...
2. [Optional]Connect your argocd cli to the ArgoCD instance. See https://argo-cd.readthedocs.io/en/stable/user-guide/commands/argocd_login/ for defferent opetions.
...
14. Apply all namespaced applications listed by `create-argo-apps` command. Then sync them with
    ```
    argocd app sync <app-name>
    ```
